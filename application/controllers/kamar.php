...
class Kamar extends CI_Controller {
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "admin/moduls/kamar";
        $this->load->view('admin/admin_view',$data);
    }
	
	public function ajax_list() {
		$list = $this->Mdl_kamar->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		foreach ($list as $kamar) {
			...
			$row[] = $kamar->deskripsi_kamar;
			$row[] = $kamar->hight_kamar;
			$row[] = $kamar->low_kamar;
			$row[] = '
			<div class="btn-group">
                        <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">Aksi <span class="caret"></span></button>
                        <ul class="dropdown-menu" role="menu">
                            <li><a href="javascript:void(0)" onclick="edit('."'".$kamar->id_kamar."'".')">Edit</a></li>
                            <li><a href="javascript:void(0)" onclick="hapus('."'".$kamar->id_kamar."'".')">Delete</a></li>
							<li class="divider"></li>
							<li><a href="kamar/'.$kamar->id_kamar.'/detail">Gambar</a></li>
                        </ul>
            </div>';
			$data[] = $row;
		}
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_kamar->count_all(),
						"recordsFiltered" => $this->Mdl_kamar->count_filtered(),
						"data" => $data,
				);
		echo json_encode($output);
	}
	
	function kamar_detail($key){
    	$row = $this->Mdl_kamar->get_by($key);

    	if (!empty($row)) {
            $data = array(
            	'id_kamar' => $row->id_kamar,
				'id_gambar_kamar' => $row->id_gambar_kamar
            );
		$data['view_file']  = "admin/moduls/detail_kamar";
        $this->load->view('admin/admin_view',$data);
        } else {
            $data['view_file']  = "admin/moduls/detail_kamar";
        	$this->load->view('admin/admin_view',$data);
	...
	public function ajax_add() {
		...
		$nama_file = $this->input->post('nama_kamar').'.'.$olah[1];
		
		$gambar = str_replace(' ', '_', $nama_file);

		$judul = $this->input->post('nama_kamar');
		$title_meta = $this->input->post('kamar_title_meta');
		$deskripsi_meta = $this->input->post('kamar_deskripsi_meta');
		$keyword_meta = $this->input->post('kamar_keyword_meta');
		//$gambar = str_replace(' ', '_', $nama_file);

		$config['upload_path'] = realpath('assets/images/kamar');
		...
		if(empty($gambar)){
 			$data = array(
			'nama_kamar' => $judul,
			'kamar_title_meta' => $title_meta,
			'kamar_deskripsi_meta' => $deskripsi_meta,
			'kamar_keyword_meta' => $keyword_meta,
		...
		$this->Mdl_kamar->add($data);
		echo json_encode(array('status' => TRUE));
	}
	
	public function ajax_edit($id) {
		$data = $this->Mdl_kamar->get_by_id($id);
